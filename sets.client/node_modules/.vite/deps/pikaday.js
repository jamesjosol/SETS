import { r as __require, t as __commonJSMin } from "./chunk-DtKSR37_.js";
//#region node_modules/pikaday/pikaday.js
var require_pikaday = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/*!
	* Pikaday
	*
		...
		*/
		var hasMoment = typeof moment === "function", hasEventListeners = !!window.addEventListener, document = window.document, sto = window.setTimeout, addEvent = function(el, e, callback, capture) {
			...
			for (prop in from) {
				...
				if (hasProp && typeof from[prop] === "object" && from[prop] !== null && from[prop].nodeName === void 0) if (isDate(from[prop])) {
			...
			if (opts.isEmpty) if (opts.showDaysInNextAndPreviousMonths) {
				arr.push("is-outside-current-month");
				if (!opts.enableSelectionDaysInNextAndPreviousMonths) arr.push("is-selection-disabled");
			...
			var jan4th = new Date(date.getFullYear(), 0, dayInFirstWeek), daysBetween = (date.getTime() - jan4th.getTime()) / (1440 * 60 * 1e3);
			return 1 + Math.round((daysBetween - dayShift + prevWeekDay(jan4th.getDay())) / daysPerWeek);
		}, renderWeek = function(d, m, y, firstWeekOfYearMinDays) {
			var date = new Date(y, m, d);
			return "<td class=\"pika-week\">" + (hasMoment ? moment(date).isoWeek() : isoWeek(date, firstWeekOfYearMinDays)) + "</td>";
			...
			self._onKeyChange = function(e) {
				e = e || window.event;
				if (self.isVisible()) switch (e.keyCode) {
					case 13:
					case 27:
			...
			self._parseFieldValue = function() {
				...
				} else return new Date(Date.parse(opts.field.value));
			};
			...
			if (opts.keyboardInput) addEvent(document, "keydown", self._onKeyChange);
			if (opts.field) {
			...
			else self.gotoDate(/* @__PURE__ */ new Date());
			if (opts.bound) {
				this.hide();
		...
		*/
		Pikaday.prototype = {
			...
			adjustDate: function(sign, days) {
				...
				var newDay;
				if (sign === "add") newDay = new Date(day.valueOf() + difference);
				else if (sign === "subtract") newDay = new Date(day.valueOf() - difference);
				this.setDate(newDay);
			},
			...
			draw: function(force) {
				...
				if (opts.bound) {
					if (opts.field.type !== "hidden") sto(function() {
						opts.trigger.focus();
					}, 1);
				}
				if (typeof this._o.onDraw === "function") this._o.onDraw(this);
				if (opts.bound) opts.field.setAttribute("aria-label", opts.ariaLabel);
			},
			...
			render: function(year, month, randId) {
				...
				for (var i = 0, r = 0; i < cells; i++) {
					...
					if (opts.pickWholeWeek && isSelected) isWeekSelected = true;
					row.push(renderDay(dayConfig));
					if (++r === 7) {
						if (opts.showWeekNumber) row.unshift(renderWeek(i - before, month, year, opts.firstWeekOfYearMinDays));
			...
			hide: function() {
				...
				if (v !== false) {
					...
					this._v = false;
					if (v !== void 0 && typeof this._o.onClose === "function") this._o.onClose.call(this);
				}
			},
			destroy: function() {
				var opts = this._o;
				...
				if (opts.keyboardInput) removeEvent(document, "keydown", this._onKeyChange);
				if (opts.field) {
...
//#endregion
export default require_pikaday();
