                ...
                <td class="px-8 py-4">
                  <div class="flex items-center gap-2">
                    ...
                    <span v-if="log.isOutsideTat"
                          ...
                          style="background-color: rgba(239,68,68,0.10); color: var(--color-error);">
                      Outside TAT
                    </span>
      ...
      <div class="rounded-2xl p-5 mb-5 flex items-end gap-4 flex-wrap"
        ...
        <div class="flex items-center gap-2 pb-0.5">
          <button class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-[0.98]"
            ...
            <span class="material-symbols-outlined text-sm"
                  :class="{ 'animate-spin': bySpecimenLoading }">
      ...
      <div class="rounded-2xl p-5 mb-5 flex items-end gap-4 flex-wrap"
        ...
        <div class="flex items-center gap-2 pb-0.5">
          <button class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-[0.98]"
                  ...
                  @click="loadByBatch">
            <span class="material-symbols-outlined text-sm"
                  :class="{ 'animate-spin': byBatchLoading }">
        ...
        <div class="px-6 py-5 flex items-center justify-between flex-shrink-0"
          ...
          <div>
            <p class="text-[10px] font-bold uppercase tracking-widest mb-1"
               style="color: var(--color-text-muted);">Audit Entry</p>
        ...
        <div class="p-6 flex flex-col flex-1">
          ...
          <div class="mb-5">
            <span class="px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-tight flex items-center gap-1.5 w-fit"
              ...
              <span class="w-2 h-2 rounded-full"
                    :style="`background-color: ${getEventDot(drawerLog.eventCode)}`"></span>
          ...
          <div v-if="drawerLog.tests"
            ...
            <p class="text-[10px] font-bold uppercase tracking-widest mb-2"
               style="color: var(--color-text-muted);">Tests</p>
            <div class="flex flex-wrap gap-2">
              <span v-for="test in drawerLog.tests.split(',')"
                    :key="test"
          ...
          <template v-for="field in drawerFields" :key="field.label">
            <div v-if="field.value && field.value != '—'"
              ...
              <p class="text-[10px] font-bold uppercase tracking-widest"
                 style="color: var(--color-text-muted);">{{ field.label }}</p>
...
<script setup>
  ...
  const tabs = [
    { key: 'by-user', label: 'By User', icon: 'person_search' },
    { key: 'by-specimen', label: 'By Lab No.', icon: 'barcode_scanner' },
    { key: 'by-batch', label: 'Batch Details', icon: 'inventory_2' },
  ]
  ...
  async function loadByUser() {
    if (!userFilter.value.userID.trim()) {
      showAlert('warning', 'Missing Field', 'Please enter a User ID.')
      return
    }
    byUserLoading.value = true
    byUserError.value = null
    byUserSearched.value = true
    byUserPage.value = 1
    try {
      byUserLogs.value = await auditApi.getByUser(
        userFilter.value.userID.trim(),
        userFilter.value.dateFrom,
        userFilter.value.dateTo
      )
    } catch (err) {
      byUserError.value = err.response?.data?.message ?? 'Failed to load audit logs.'
      showAlert('error', 'Load Error', byUserError.value)
    } finally {
      byUserLoading.value = false
    }
  }
  ...
  async function loadBySpecimen() {
    if (!specimenFilter.value.specimenNo.trim()) {
      showAlert('warning', 'Missing Field', 'Please enter a specimen number.')
      return
    }
    bySpecimenLoading.value = true
    bySpecimenError.value = null
    bySpecimenSearched.value = true
    bySpecimenPage.value = 1
    try {
      ...
      bySpecimenLoading.value = false
    }
  }

  function clearBySpecimen() {
    specimenFilter.value = { specimenNo: '' }
    bySpecimenLogs.value = []
    bySpecimenError.value = null
    bySpecimenSearched.value = false
    bySpecimenPage.value = 1
  }
  ...
  async function loadByBatch() {
    if (!batchFilter.value.batchNo.trim()) {
      showAlert('warning', 'Missing Field', 'Please enter a batch number.')
      return
    }
    byBatchLoading.value = true
    byBatchError.value = null
    byBatchSearched.value = true
    byBatchPage.value = 1
    try {
