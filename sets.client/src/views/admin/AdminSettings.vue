          ...
          <div class="px-8 py-6 space-y-5">
            ...
            <AppTable :rows="filteredUsers"
              ...
              <template #cell-isAdmin="{ value }">
                <span v-if="value" class="material-symbols-outlined text-base"
                ...
                <div class="px-6 py-5 space-y-5">
                  ...
                  <div v-if="rdModal.mode === 'add'">
                    ...
                    <div class="relative">
                      ...
                      <div class="absolute right-3 top-1/2 -translate-y-1/2">
                        ...
                        <span v-else-if="rdModal.form.testCode"
                              class="material-symbols-outlined text-sm" style="color: var(--color-primary);">check_circle</span>
                    ...
                    <div v-if="rdModal.form.testCode"
                      ...
                      <button class="material-symbols-outlined text-sm" style="color: var(--color-text-muted);"
                              @click="clearRdTest">
                        close
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <p v-if="rdModal.dayError" class="mb-2 text-xs font-bold" style="color: #ba1a1a;">
          ...
          <div class="px-8 py-6 space-y-8">
            ...
            <div class="flex items-center justify-between p-5 rounded-2xl"
              ...
              <div>
                ...
                <p class="text-xs mt-0.5" style="color: var(--color-text-muted);">
                  When enabled, lab section users can scan On-Site / Mission specimens on the Assign RMT page.
                </p>
              </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p class="text-xs mt-0.5" style="color: var(--color-text-muted);">
                    Characters 3–4 of the lab number must match an active prefix to be allowed for scanning.
                  </p>
                </div>
              </div>
              ...
              <!-- Error -->
              <p v-if="onSiteAddError" class="text-xs mb-3 font-semibold" style="color: var(--color-error);">
                {{ onSiteAddError }}
              </p>

              <!-- Prefix table -->
              <div v-if="onSiteLoading" class="py-10 text-center text-sm" style="color: var(--color-text-muted);">
                Loading...
              </div>
              <div v-else-if="!onSiteLabNos.length" class="py-10 text-center text-sm" style="color: var(--color-text-muted);">
                No prefixes configured yet.
              </div>
              <table v-else class="w-full text-sm">
                <thead>
                  <tr style="border-bottom: 1px solid var(--color-border);">
                    <th class="text-left px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest" style="color: var(--color-text-muted);">Prefix</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest" style="color: var(--color-text-muted);">Description</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest" style="color: var(--color-text-muted);">Status</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest" style="color: var(--color-text-muted);">Added By</th>
                    <th class="px-4 py-2.5"></th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="entry in onSiteLabNos" :key="entry.id"
                      style="border-bottom: 1px solid var(--color-border);">
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">
                      <div class="flex items-center gap-2 justify-end">
                        ...
                        <button class="text-xs font-bold px-3 py-1.5 rounded-lg transition-all"
                                style="background-color: rgba(239,68,68,0.1); color: var(--color-error, #ef4444);"
...
const filteredRd = computed(() => {
  ...
  return rdList.value.filter(r =>
    r.testCode.toLowerCase().includes(q) || r.testName.toLowerCase().includes(q)
  )
})
