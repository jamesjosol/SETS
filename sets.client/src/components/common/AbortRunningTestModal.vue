  ...
  <div v-if="isVisible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md rounded-2xl shadow-2xl p-6 flex flex-col gap-4 animate-modal"
      ...
      <div class="flex items-center gap-3">
        <span class="material-symbols-outlined text-2xl" style="color: var(--color-warning);">undo</span>
        <div>
          <p class="font-bold text-sm" style="color: var(--color-text);">Abort Running Test</p>
          <p class="text-xs font-mono mt-0.5" style="color: var(--color-text-muted);">
            {{ specimenNo }} · {{ testCode }} — {{ testName }}
          </p>
        </div>
      </div>
      ...
      <p class="text-xs" style="color: var(--color-text-muted);">
        This will reset the test back to pending and clear its RMT assignment. The specimen will reappear in the Pending Specimens list.
      </p>
      ...
      <div class="flex gap-3 justify-end">
        ...
        <button class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-95 disabled:opacity-40"
          ...
          <span v-else>Confirm Abort</span>
        </button>
      </div>
