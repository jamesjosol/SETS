<template>
  <div v-if="isVisible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <!-- Modal -->
    <div class="relative w-full max-w-sm rounded-2xl shadow-2xl p-8 flex flex-col items-center text-center gap-4 animate-modal"
        style="background-color: var(--color-base-100)">
      <!-- Animated Icon -->
      <div class="w-20 h-20 rounded-full flex items-center justify-center mb-2"
           :style="iconBgStyle">
        <!-- Error Icon -->
        <svg v-if="type === 'error'"
      ...
      <!-- Title -->
      <h3 class="text-xl font-bold" style="color: var(--color-base-content)">{{ title }}</h3>

      <!-- Message -->
      <p class="text-sm leading-relaxed" style="color: var(--color-base-content/60)">{{ message }}</p>

      <!-- Confirm Button -->
      <button class="mt-2 w-full py-3 px-6 rounded-xl font-bold uppercase tracking-widest text-sm transition-all active:scale-[0.98]"
              :style="buttonStyle"
              @click="handleConfirm">
        {{ confirmText }}
      </button>
