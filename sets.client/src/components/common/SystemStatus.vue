...
function applyState(index, online, latencyMs = 0) {
  ...
  if (online) {
    if      (latencyMs >= 1000) { item.state = 'Severe Delay'; item.iconColor = '#ea580c' }
    else if (latencyMs >= 500) { item.state = 'Delay';        item.iconColor = '#d97706' }
    else if (latencyMs >= 200)  { item.state = 'Slight Delay'; item.iconColor = '#ca8a04' }
